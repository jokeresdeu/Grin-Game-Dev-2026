...
namespace RPG
{
    public class Player : MonoBehaviour
    {
        ...
        [SerializeField] private float _fireballManaCost = 50f;
        ...
        private void Start()
        {
            if (RPGGameManager.Instance != null)
            {
                ...
                RPGGameManager.Instance.UpdateManaUI(_currentMana, _maxMana);
            }
        ...
        private void UpdateAnimations()
        {
            ...
            bool isArenaPaused = RPGGameManager.Instance != null && RPGGameManager.Instance.IsArenaPaused;
        ...
        {
            if (Input.GetKeyDown(KeyCode.F) && _currentMana >= _fireballManaCost)
            {
                PerformMagic();
            }
        }
        ...
        private void PerformMagic()
        {
            ...
            if (_animator != null)
            {
                _animator.Play("Cast1H", 0);
            }

            if (_fireballPrefab != null)
            {
                Vector3 spawnPos = transform.position + Vector3.up * 1.5f + transform.right * 1f;
                GameObject fb = Instantiate(_fireballPrefab, spawnPos, transform.rotation);
                
                var proj = fb.GetComponent<FireballProjectile>();
                Rigidbody2D rb = fb.GetComponent<Rigidbody2D>();
                if (rb != null)
                {
                    rb.linearVelocity = transform.right * _fireballSpeed;
                }
            }
        }
